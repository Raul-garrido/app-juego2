...
- **Sistema herramienta + color de variante, con fotos**: 27 herramientas — las 15 originales (manuales) mas 12 nuevas, sobre todo electricas (amoladora, compresor, esmeriladora de banco, lijadora, nivel laser, multiherramienta oscilante, pistola de calor, herramienta rotativa, sierra circular, sierra sable, soplete, tornillo de banco), recortadas de 3 collages nuevos que el usuario mando con bastante repeticion entre ellos — se descartaron a mano las herramientas duplicadas o demasiado parecidas a las que ya teniamos (ver `NOTICE.md`). Cada foto se recolorea con un filtro CSS `hue-rotate` (6 variantes) — funciona bien porque el metal de las fotos esta poco saturado y el mango/mordaza es el color dominante, asi que solo se recolorea la parte que "deberia" cambiar. Da hasta 162 tipos de ficha combinando foto × color. Añadir una herramienta mas es una foto nueva recortada a cuadrado + una entrada en `tools.ts`.
...
- **Acabado 3D real de la ficha**: bisel marfil con degradado, sombra dura de "grosor" y brillo superior (CSS) — se nota especialmente en como asoma la ficha tapada por detras de la de arriba. Hay una vista de repaso en `/?gallery` que pinta las 15 herramientas x los 6 colores para revisarlas de un vistazo cuando se añadan mas.
- **Pendiente de pulir**: un par de fotos (llaves Allen, alicate de punta) quedan pequeñas/finas dentro de la ficha por como estaba recortado el collage original; recortarlas mas ajustadas a la herramienta cuando se retoque el arte.
