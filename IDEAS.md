...
- **Sistema herramienta + color de variante, con fotos**: 15 herramientas (martillo, destornillador, alicates, alicate de punta, llave de tubo, llaves Allen, cinta metrica, nivel, cutter, cizalla, tijera de chapa, tijera de podar, juego de brocas, estuche de llaves, multimetro). Cada foto se recolorea con un filtro CSS `hue-rotate` (6 variantes) — funciona bien porque el metal de las fotos esta poco saturado y el mango/mordaza es el color dominante, asi que solo se recolorea la parte que "deberia" cambiar. Da hasta 90 tipos de ficha combinando foto × color. Añadir una herramienta mas es una foto nueva recortada a cuadrado + una entrada en `tools.ts`.
